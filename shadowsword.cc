...
class VeryDumbHuman : public Human {
	...
	bool want;
public:
    VeryDumbHuman(std::string const& name_): name(name_), want(false) {}

    std::string const& getName() const { return name; }
    bool wantBuy(std::string const& property) { want = !want; return want; }
    bool wantSell(std::string const& property) { want = !want; return want; }
